...
def GVC2_reconstruction(P_gh, n_ghost, nx_domain):
    ...
    for k_var in range(3):
        ...
        for j_inter in range(nx_domain + 1):
            ...
            is_left_of_discontinuityl = abs(q_0L - q_m1L) < abs(q_p1L - q_0L)
    
            if is_left_of_discontinuityl: # Upwind-biased, "fast"
                q_interfacel = (3.0 * q_0L - q_m1L) / 2.0
            else: # Central-like, "slow"
                q_interfacel = (q_p1L + q_0L) / 2.0
            ...
            is_right_of_discontinuityr = abs(q_0R - q_m1R) < abs(q_p1R - q_0R)   
            if is_right_of_discontinuityr:       
                q_interfacer = (3.0 * q_0R - q_m1R) / 2.0
            else:
                q_interfacer = (q_p1R + q_p1R) / 2.0
