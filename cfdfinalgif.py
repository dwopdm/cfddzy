...
T_FINAL_STATIC_PLOT = 0.14 # Final simulation time FOR STATIC PLOTS
...
def GVC2_reconstruction(P_gh, n_ghost, nx_domain):
    ...
    for k_var in range(3):
        q = P_gh[k_var, :]
        for j_inter in range(nx_domain + 1):
            # Left state q_{L, j_inter+1/2} at interface j_inter (to the right of cell idx_L_cell)
            idx_L_cell = n_ghost + j_inter - 1 # This is cell 'i' in paper's notation for interface i+1/2
            q_im1L = q[idx_L_cell-1] # q_{i-1}
            q_iL   = q[idx_L_cell]   # q_i
            q_ip1L = q[idx_L_cell+1] # q_{i+1}
            
            K_iL   = np.abs(q_iL - q_im1L)
            K_ip1L = np.abs(q_ip1L - q_iL)

            if K_iL < K_ip1L: # Condition for "fast" reconstruction from left (upwind)
                q_interfacel = (3.0 * q_iL - q_im1L) / 2.0
            else: # Condition for "slow" reconstruction from left (central)
                q_interfacel = (q_ip1L + q_iL) / 2.0
            P_L_at_interfaces[k_var, j_inter] = q_interfacel
            
            # Right state q_{R, j_inter+1/2} at interface j_inter (to the left of cell idx_R_cell)
            idx_R_cell = n_ghost + j_inter # This is cell 'i+1' in paper's notation for interface i+1/2
            q_iR   = q[idx_R_cell-1] # q_i (cell to the left of idx_R_cell)
            q_ip1R = q[idx_R_cell]   # q_{i+1} (cell idx_R_cell itself)
            q_ip2R = q[idx_R_cell+1] # q_{i+2} (cell to the right of idx_R_cell)

            K_ip1R = np.abs(q_ip1R - q_iR)
            K_ip2R = np.abs(q_ip2R - q_ip1R)

            if K_ip1R > K_ip2R: # Condition for "fast" reconstruction from right (upwind)
                q_interfacer = (3.0 * q_ip1R - q_ip2R) / 2.0
            else: # Condition for "slow" reconstruction from right (central)
...
def run_simulation_single(U_init,t_fin,dx_s,cfl,nx_s,recon,flux,lim=None,gam=gamma,verb=True):
    ...
    while t_c<t_fin:
        ...
        dt=cfl*dx_s/max_wave_speed if max_wave_speed>1e-9 else cfl*dx_s
...
def run_simulation_and_collect_frames(U_init_sim, t_final_anim, dx_sim, cfl_val, nx_val,
    ...
    while t_curr < t_final_anim:
        ...
        if dt_val <= 1e-12: break # Time step too small or already at t_final_anim
        ...
        # Frame capture logic: Store frame if current sim time has crossed the target frame time
        # This stores the state *at t_new* for all target frame times between t_curr and t_new.
        # This is simpler than interpolation and usually good if dt_val << anim_frame_dt_target.
        while next_frame_capture_time <= t_new + 1e-9 and next_frame_capture_time <= t_final_anim + 1e-9:
            rho_f, u_f, p_f = conserved_to_primitive(U_new, gamma_val) # Use U_new for the frame
            e_f = p_f / ((gamma_val - 1.0) * np.maximum(rho_f, 1e-9))
            animation_frames.append({
                't': next_frame_capture_time, # Label frame with target time
    ...
    # Ensure one last frame at t_final_anim if not already captured
    if t_final_anim > 0 and (not animation_frames or abs(animation_frames[-1]['t'] - t_final_anim) > 1e-6) and \
       (next_frame_capture_time > t_final_anim or abs(next_frame_capture_time - (t_final_anim + anim_frame_dt_target)) < 1e-6) :
        rho_f, u_f, p_f = conserved_to_primitive(U, gamma_val)
        e_f = p_f / ((gamma_val - 1.0) * np.maximum(rho_f, 1e-9))
        animation_frames.append({
            't': t_final_anim, 'rho': np.copy(rho_f), 'u': np.copy(u_f),
            'p': np.copy(p_f), 'e': np.copy(e_f)
        })
...
if __name__ == "__main__":
    ...
    if num_schemes <= 10: cmap = plt.colormaps.get_cmap('tab10')
    elif num_schemes <= 20: cmap = plt.colormaps.get_cmap('tab20')
    else: cmap = plt.colormaps.get_cmap('viridis', num_schemes)
    
    for i, config in enumerate(schemes_to_test):
        config['color'] = cmap(i / (num_schemes -1 ) if num_schemes > 1 else 0.5)
    ...
    ANIM_T_FINAL = 0.25 # Adjusted for more focused animation, original was 5
    ANIM_FRAME_DT_TARGET = 0.0025 # Adjusted, original was 0.01
    ANIM_FPS = 30 # Adjusted FPS, original was 60
    ...
    if CREATE_ANIMATION:
        ...
        if not all_animation_frames_data or min_frames_collected < 2:
            ...
            anim_times_for_ylim = np.linspace(0, ANIM_T_FINAL, max(20, num_render_frames // 5)) # More samples for ylim
            for t_sample in anim_times_for_ylim:
                ...
                for key in plot_vars_keys:
                    min_max_for_ylim[key][0]=min(min_max_for_ylim[key][0], np.min(ex_s_data[key][np.isfinite(ex_s_data[key])]))
                    min_max_for_ylim[key][1]=max(min_max_for_ylim[key][1], np.max(ex_s_data[key][np.isfinite(ex_s_data[key])]))
            
            for config in schemes_to_test: # Check numerical scheme data too
                frames = all_animation_frames_data[config['label']]
                if len(frames) >= num_render_frames: 
                    indices_to_check = np.linspace(0, num_render_frames-1, 10, dtype=int) # Check a few frames
                    for frame_idx_check in indices_to_check:
                        f_data = frames[frame_idx_check]
                        for key in plot_vars_keys:
                            min_max_for_ylim[key][0]=min(min_max_for_ylim[key][0],np.min(f_data[key][np.isfinite(f_data[key])]))
                            min_max_for_ylim[key][1]=max(min_max_for_ylim[key][1],np.max(f_data[key][np.isfinite(f_data[key])]))
            ...
            for key in plot_vars_keys:
                ...
                if not np.isfinite(low_lim): low_lim = 0
                if not np.isfinite(upp_lim): upp_lim = low_lim + 1.0 # Default if limits are bad
                ax.set_ylim(low_lim, upp_lim); ax.set_xlim(XMIN, XMAX)

            axes_anim['rho'].legend(fontsize='xx-small', loc='upper right', ncol=2)
            fig_anim.suptitle('', fontsize=16) # Initial empty title
            ...
            for cfg_item in schemes_to_test:
                if len(all_animation_frames_data[cfg_item['label']]) >= num_render_frames:
                    ...
                    break
            if ref_scheme_label_for_time is None: 
                print("Error: Could not find a reference scheme for animation timing. Using first scheme.")
            ...
            def update_anim_plot_multi(frame_idx):
                ...
                artists_changed.append(fig_anim.texts[0]) # Add suptitle to list of artists to redraw
        ...
        # --- Part 3: Generate Individual Animations ---
        ...
        if CREATE_INDIVIDUAL_ANIMATIONS and all_animation_frames_data:
            ...
            for config_individual in schemes_to_test:
                ...
                for key_idx, key_var in enumerate(plot_vars_keys):
                    ...
                    min_v, max_v = min_max_for_ylim[key_var] 
                    span = max_v - min_v
                    if not np.isfinite(span) or span < 1e-6*abs(max_v) or span < 1e-6: span = abs(max_v)*0.2 if abs(max_v)>1e-6 else 0.2
                    if not np.isfinite(span) or span < 1e-6: span = 0.2
                    low_lim=min_v-0.1*span; upp_lim=max_v+0.1*span
                    if key_var in ['rho','p','e']: low_lim=max(0,low_lim if np.isfinite(low_lim) else 0)
                    if not np.isfinite(low_lim): low_lim = 0
                    if not np.isfinite(upp_lim): upp_lim = low_lim + 1.0
                    ax.set_ylim(low_lim, upp_lim); ax.set_xlim(XMIN, XMAX)
                
                axes_anim_indiv['rho'].legend(fontsize='small', loc='upper right')
                fig_anim_indiv.suptitle('', fontsize=16) 
                fig_anim_indiv.tight_layout(rect=[0, 0.03, 1, 0.95])

                def update_anim_plot_individual(frame_idx_indiv):
                    if frame_idx_indiv >= len(frames_data_indiv): return [] 
